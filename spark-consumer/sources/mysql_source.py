"""
MySQLSource — production-grade batch extractor for MySQL → Iceberg / Postgres.

Load strategy:
  ┌───────────────────────────────────────────────────────────────────────────┐
  │ Destination table does NOT exist (or cursor_field is None)               │
  │   → FULL LOAD: SELECT * FROM source_table                                │
...
After fetching:
  - Null bytes (\x00) stripped from all string columns.
  - Empty strings normalized to NULL.
  - audit_updated_date (IST) added.
  - All column names lowercased.
  - Schema reconciled against destination (new columns added to Iceberg,
    missing destination columns padded as NULL, void columns cast).
"""
...
class MySQLSource:
    """
    Reads a MySQL table and loads it into an Iceberg or Postgres destination.
    ...
    def ingest(
        ...
        Args:
            source_table:  Table name in MySQL to read from.
        ...
        start = datetime.now()
        logger.info(
            "MySQLSource.ingest: %s.%s → %s.%s.%s [writer=%s]",
            self.database, source_table, catalog, namespace, dest_table, writer_type,
        )
        ...
        if not self._source_table_exists(source_table):
            logger.warning(
                "Source table %s.%s not found — skipping.", self.database, source_table
            )
            return
        ...
        count = df.count()
        logger.info("Fetched %d rows from %s.%s", count, self.database, source_table)
        ...
        logger.info("MySQLSource.ingest complete in %.2f min", elapsed)
    ...
    def _source_table_exists(self, table_name: str) -> bool:
        try:
            ...
            logger.error("Error checking MySQL table existence: %s", exc)
            return False
    ...
    def _clean(df):
        from pyspark.sql.functions import (
            col, expr, from_utc_timestamp, regexp_replace, trim, when,
        )
        from pyspark.sql.types import StringType

        # Remove null bytes that break UTF-8 encoding

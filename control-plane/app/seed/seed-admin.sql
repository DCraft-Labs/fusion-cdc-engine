...
BEGIN
  ...
  --
  --    NOTE: the `destinations` table has NO host/port/database_name/etc columns —
  --    all connection fields live inside the `connection_config` JSONB column
  --    (see schemas/schema_postgres.sql:154 and app/models/source_destination.py).
  --    The v1.2.1 seed INSERT listed host/port/database_name/schema_name/username/
  --    password_encrypted/ssl_enabled/ssl_config/config as top-level columns, which
  --    do NOT exist → the atomic DO block failed with "column "host" does not
  --    exist", rolling back the entire seed (including connector_definitions) and
  --    leaving the CDC UI with total: 0 everywhere. Fixed in v1.2.2.
  ...
  SELECT
    ...
    '{
      ...
      "schema_name": "public",
      "username": "dw_user",
      "password_encrypted": "gAAAAABp_L8o4jvV4fglnSnabhrkN5vcXJn3RjjSF3X_YFQ5YXkLrNIxZwe72Qj1SczIdTpQYib4usUoSkBxXuyJNjeqio-k6w==",
      "ssl_enabled": false,
      "ssl_config": {},
  ...
  --    All destination-specific config goes into connection_config JSONB.
  --    (uses WHERE NOT EXISTS guard — see note on sources above.)
  -- ──────────────────────────────────────────────────────────────────────────
  INSERT INTO destinations (
    destination_name, connector_definition_id, connector_version,
  ...
  --
  --    NOTE: the `connections` table has NO sync_enabled/replication_slot/
  --    publication/namespace_definition/namespace_format/stream_prefix/config
  --    columns (see schemas/schema_postgres.sql:306 and
  --    app/models/connection.py). The v1.2.1 seed INSERT listed them as
  --    top-level columns, which do NOT exist → the atomic DO block failed and
  --    rolled back the entire seed. replication_slot/publication belong on the
  --    SOURCE config (already set in the sources INSERT above), not the
  --    connection. Fixed in v1.2.2.
  --    (uses WHERE NOT EXISTS guard — see note on sources above.)
  -- ──────────────────────────────────────────────────────────────────────────
